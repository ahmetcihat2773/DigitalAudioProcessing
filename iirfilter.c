...
REFERENCES : 
 ...
 1) http://truelogic.org/wordpress/2015/09/04/parsing-a-wav-file-in-c/
 2) https://codereview.stackexchange.com/questions/151049/endianness-conversion-in-c
 3) https://wavefilegem.com/how_wave_files_work.html
*/
#include "fdacoefs.h"
#include <stdio.h>
#include <string.h>
...
void iir_filter_typeII ( int16_t *input_signal,int num_samples, int16_t *channel_filter)
{
	...
	// Multiply with the max to convert it to the integer.	
...
void iir_filter_typeI ( int16_t *input_signal,int num_samples, int16_t *channel_filter)
{
float shift_registerX1[MWSPT_NSEC][3];
float shift_registerX2[MWSPT_NSEC][3];
float shift_registerY1[MWSPT_NSEC][3];
float shift_registerY2[MWSPT_NSEC][3];
int m,n;

for(m=0; m< MWSPT_NSEC; m++){
	for(n=0;n<3;n++){
		
		shift_registerX1[MWSPT_NSEC][3] = 0.0;
		shift_registerX2[MWSPT_NSEC][3] = 0.0;
		shift_registerY1[MWSPT_NSEC][3] = 0.0;
		shift_registerY2[MWSPT_NSEC][3] = 0.0;
	}
}
...
for (k = 0; k < num_samples-1; k++) 
{	
	input = (float)input_signal[k];					
	
	for(i=0;i<MWSPT_NSEC;i++){		
	// Here the shift register logic is implemented. Each structure has its own shift register according to this website
	// https://www.keil.com/pack/doc/CMSIS/DSP/html/group__BiquadCascadeDF1.html  It is implemented accordingly. 
		
		temp = input*b[i][0] + b[i][1]*shift_registerX1[i][0] + b[i][2]*shift_registerX2[i][1];
		y = a[i][0]*temp - a[i][1]*shift_registerY1[i][0] - a[i][2]*shift_registerY2[i][1];	
		shift_registerX2[i][1] = shift_registerX1[i][0];
		shift_registerX1[i][0] = input; 
		shift_registerY2[i][1] = shift_registerY2[i][0];
		shift_registerY1[i][0] = y;

	}
		
	channel_filter [k] = int16_sat(y);
}
...
int main(int argc, char **argv) {
	...
	iir_filter_typeII(channel_zero,num_samples,channel_zero_filter);
	iir_filter_typeII(channel_one,num_samples,channel_one_filter);
	int k;
